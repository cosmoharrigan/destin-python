...
class RecNode(Node):
    """
    Recurrent clustering subclass. This is not currently being used.
    Documentation will be updated when it is. 1/9/2014
    """
    ...
    def rec_init(self, mr, vr, sr, di, ce, node_id):
        self.EXTDIMS = di
        di += ce
        self.common_init(mr, vr, sr, di, ce, node_id)
    ...
    def update_node(self, input, TRAIN):
        #print self.ID
        #print input
        input = np.concatenate([input.reshape(1, self.EXTDIMS), self.belief], axis=1)
        self.process_input(input, TRAIN)
